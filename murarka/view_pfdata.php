    ...
    if(!empty($_FILES['file']['name']) && in_array($_FILES['file']['type'],$csvMimes)){
        if(is_uploaded_file($_FILES['file']['tmp_name'])){
            ...
            while(($line = fgetcsv($csvFile)) !== FALSE){
               ...
               //  {
                      $result=mysqli_query($link, "UPDATE `pf_data_demo` SET 
                                                `particulars`='".$line[1]."',
                                                `d_employee_share`='".stripslashes($line[2])."',
                                                `d_employer_share`='".stripslashes($line[3])."',
                                                `w_employee_share`='".stripslashes($line[4])."',
                                                `w_employer_share`='".stripslashes($line[5])."',
                                                `contribution`='".stripslashes($line[6])."',
                                                `edited_date`='".date("Y-m-d")."',
                                                `edited_by_id`='".$user_id."'
                                         WHERE `user_master_id`='".$user_master_excel['user_master_id']."' AND
                                               `month`='".stripslashes($line[7])."' AND
                                               `year`='".stripslashes($line[8])."';");
                //}
...
if(isset($_REQUEST['edit_submit']))
{
    ...
    $contribution=stripslashes($_POST['contribution']);
        ...
        $edit_pf_result=mysqli_query($link, "UPDATE `pf_data_demo` SET 
...
{
    $delete_pf_result=mysqli_query($link, "UPDATE `pf_data_demo` SET 
                                      `status`=1,
...
//   echo  $query_coppy="INSERT INTO pf_data ( 
...
//   if(mysql_query($link, $query_coppy))
//   {
...
//                                 pf_data_demo.`user_master_id`=pf_data.`user_master_id` AND
//                                 pf_data_demo.`status`=pf_data.`status`));";
//       if(mysql_query($link, $demo_delete))
//       {
...
if($_REQUEST['key']=="approved" && $_REQUEST['month']>0 && $_REQUEST['year']>0)
{
  ...
  if(mysqli_query($link, $query_coppy))
  {
      $demo_delete="DELETE FROM pf_data_demo
                    WHERE 
                          pf_data_demo.`month`='".$_REQUEST['month']."' AND
                          pf_data_demo.`year`='".$_REQUEST['year']."' AND 
                          pf_data_demo.`status`=0;";
      if(mysqli_query($link, $demo_delete))
      {
        echo "<script>alert('PF Data Successfully Approved');
            window.location = 'view_pfdata_approved';</script>";
      }
            ...
            <label class="col-sm-2 control-label">EFRL Code</label>
              <div class="col-sm-2 controls">
                <select class="form-control input-sm" name="user_master_id" id="user_master_id" onChange="send_emp_code(this.value)">
                  ...
                  <?php 
                  $deperment_query=mysqli_query($link, "SELECT `user_master_id`,`efrl_code` FROM `user_master` WHERE `status`=0 AND `job_status`=0 AND `department_id`>4 ORDER BY `efrl_code` ASC;");
