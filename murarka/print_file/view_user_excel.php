...
header("Content-Disposition: attachment; filename=view_user_".date('Y-m-d').".xls");
...
<h2><i class="fa fa-indent"></i> USER INFO</h2>
</div>
<!-- END Page Title -->
...
<table border="1" name="dynamic">
  <tbody>
    <tr height="20">
      <td><strong>efrl_code</strong></td>
      <td><strong>user_master_id</strong></td>      
    </tr>   
  </tbody>
<?php
      $user_query=mysqli_query($link, "SELECT `user_master_id`,`efrl_code` FROM `user_master` WHERE `status`=0 AND `department_id`>4 ;");
      while ($user_data=mysqli_fetch_array($user_query)) {
      ?>
      <tr>          
          <td><?php echo $user_data['efrl_code']; ?></td>
          <td><?php echo $user_data['user_master_id']; ?></td>
       </tr>
      <?php
    }
  ?>
