            ...
            <label class="col-sm-2 control-label">Employee Code.</label>
              <div class="col-sm-2 controls">
                <select class="form-control input-sm" name="user_master_id" id="user_master_id" onChange="send_emp_code(this.value)">
                  ...
                  <?php 
                  $deperment_query=mysql_query("SELECT `user_master_id`,`efrl_code` FROM `user_master` WHERE `status`=0 AND `job_status`=0 ORDER BY `efrl_code` ASC;");
            ...
            <label class="col-sm-2 control-label">Accounting Year</label>
              <div class="col-sm-2 controls">
                 <select class="form-control input-sm" name="act_year" id="act_year" onChange="send_year(this.value)">
                  <option value="">-- Select Acc Year--</option>
                    
                  <option value="18-19" <?php if($_REQUEST['act_year']=="18-19") { echo "selected"; } ?>>2018-19</option>
                  <option value="19-20" <?php if($_REQUEST['act_year']=="19-20") { echo "selected"; } ?>>2019-20</option>
  ...
  <div class="box box-info">
    <div class="box-header" style="padding: 5px"> 
      ...
      <?php
        ...
        {
      ?>
        <a class="btn btn-xs btn-success pull-right" style="margin-right: 20px" title="Excel Download" href="print_file/view_pfapproved_excel?user_master_id=<?=$user_master_id?>&month=<?=$_REQUEST['month']?>&act_year=<?=$_REQUEST['act_year']?>" title="Export to Excel"><i class="fa fa-file-excel-o"></i></a>

        <a class="btn btn-xs btn-info pull-right" style="margin-right: 5px" title="Print" href="print_file/print_pfdata?user_master_id=<?=$user_master_id?>&month=<?=$_REQUEST['month']?>&act_year=<?=$_REQUEST['act_year']?>"><i class="fa fa-print"></i></a>
        <?php
          }
        ?>
    </div>
    ...
    if($_REQUEST['act_year']=="18-19")
    {
          ...
          ?>
           ...
           <?php } 
            ...
            $arrear_query=mysql_query("SELECT * FROM `pf_data` WHERE pf_data.`status` = 0 AND pf_data.`particulars`='Arrear' $key;");
            while ($arrear_result=mysql_fetch_array($arrear_query)) {            
            if($arrear_result['pf_data_id']>0){
            ?>
            <tr>
             <td><?php
              $employee5=mysql_fetch_array(mysql_query("SELECT `efrl_code` FROM `user_master` WHERE `user_master_id`='".$arrear_result['user_master_id']."' AND `status`=0;"));
            ...
            <?php
              ...
              $interest_query=mysql_query("SELECT * FROM `pf_data` WHERE pf_data.`status` = 0 AND pf_data.`particulars`='Interest for the year' $key;");
              while ($interest_result=mysql_fetch_array($interest_query)) {
              if($interest_result['pf_data_id']>0){
            ?>
            <tr>
              <?php  ?>
             <td><?php
              $employee5=mysql_fetch_array(mysql_query("SELECT `efrl_code` FROM `user_master` WHERE `user_master_id`='".$interest_result['user_master_id']."' AND `status`=0;"));
              echo $employee5['efrl_code']; ?></td>
              ...
              $opening_check=mysql_fetch_array(mysql_query("SELECT * FROM `pf_data` WHERE `act_year`='".$_REQUEST['act_year']."' AND `user_master_id`='".$user_master_id."';"));
              if($opening_check['particulars']==$opening_text)
              {
              ?>
                <tr>
                  <td><?php
                  $employee=mysql_fetch_array(mysql_query("SELECT `efrl_code` FROM `user_master` WHERE `user_master_id`='".$opening_check['user_master_id']."' AND `status`=0;"));
                  echo $employee['efrl_code']; ?></td>
      ...
      <?php  }
            ...
            ?>
              <tr>
                <td><?php
                  $employee=mysql_fetch_array(mysql_query("SELECT `efrl_code` FROM `user_master` WHERE `user_master_id`='".$user_master_id."' AND `status`=0;"));
                  echo $employee['efrl_code']; 
                  ?></td>
                  <td><?php echo $opening_text . "-" . $opening_check['year']; ?></td>
                  <td><?php $previous_acc_year="18-19"; 
                  $opening_d_employee_share=mysql_fetch_array(mysql_query("SELECT SUM(`d_employee_share`) AS `d_employee_share` FROM `pf_data` WHERE `act_year`='".$previous_acc_year."' AND `user_master_id`='".$user_master_id."';"));
                    echo $opening_d_employee_share['d_employee_share']; ?>
                  </td>
                  <td> <?php $opening_d_employer_share=mysql_fetch_array(mysql_query("SELECT SUM(`d_employer_share`) AS `d_employer_share` FROM `pf_data` WHERE `act_year`='".$previous_acc_year."' AND `user_master_id`='".$user_master_id."';"));
                    echo $opening_d_employer_share['d_employer_share']; ?>
                  </td>  
                  <td>
                    <?php $opening_w_employee_share=mysql_fetch_array(mysql_query("SELECT SUM(`w_employee_share`) AS `w_employee_share` FROM `pf_data` WHERE `act_year`='".$previous_acc_year."' AND `user_master_id`='".$user_master_id."';"));
                    echo $opening_w_employee_share['w_employee_share']; ?>
                  </td>
                  <td> 
                    <?php $opening_w_employer_share=mysql_fetch_array(mysql_query("SELECT SUM(`w_employer_share`) AS `w_employer_share` FROM `pf_data` WHERE `act_year`='".$previous_acc_year."' AND `user_master_id`='".$user_master_id."';"));
                    echo $opening_w_employer_share['w_employer_share']; ?>
                  </td>
                  <td>
                    <?php $opening_contribution=mysql_fetch_array(mysql_query("SELECT SUM(`contribution`) AS `contribution` FROM `pf_data` WHERE `act_year`='".$previous_acc_year."' AND `user_master_id`='".$user_master_id."';"));
